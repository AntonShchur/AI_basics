...
class Player:
    ...
    def update(self):
        ...
        if self.on_coin():
            self.eat_coin()
        if self.on_enemy() and not self.god:
            self.application.remove_life()
        if self.on_enemy() and self.god and self.can_eat_enemy:
            self.eat_enemy()
    ...
    def eat_coin(self):
        ...
        self.current_score += self.score_for_coin
    ...
    def eat_enemy(self):
        ...
        :return:
        """
        for index, enemy in enumerate(self.application.enemies):
            if int(enemy.position[0]) == int(self.grid_pos[0]) and int(enemy.position[1]) == int(self.grid_pos[1]):
                self.application.enemies.pop(index)
