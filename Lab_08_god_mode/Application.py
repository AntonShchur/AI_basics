...
class App:
    def __init__(self):
        ...
        self.console = PyCon.PyCon(self.screen,
                           (0, 0, MAZE_WIDTH + PADDING * 2, MAZE_HEIGHT / 6),
                           functions={"map_change_color": self.change_color,
                                      "bot_change_algo": self.change_bot_algo,
                                      "bot_stop": self.stop_bot,
                                      "player_god": self.god,
    ...
    def set_score_for_coin(self, value):
        if 10000 >= value >= 0:
            self.player.score_for_coin = value

    def eat_enemies(self, can_eat):
        if can_eat == 1:
            self.player.can_eat_enemy = True
        else:
            self.player.can_eat_enemy = False
    ...
    def stop_bot(self, *args):
        if len(args) == 0:
            for enemy in self.enemies:
                enemy.personality = STOP
        else:
            if len(self.enemies) >= args[0] > 0:
                self.enemies[args[0] - 1].personality = STOP

    def change_color(self, color):
        self.wall_color = color

    def change_bot_algo(self, bot_number,  algo):
        if len(self.enemies) >= bot_number > 0:
            if algo in [RANDOM, DEFAULT]:
                self.enemies[bot_number - 1].personality = algo
...
# This is the main block of code controls game process and gameplay
    ...
    def draw_walls(self):
        ...
        for x in range(h):
            for y in range(w):
                if maze[x, y] == WALL:
                    pygame.draw.rect(self.screen, self.wall_color, (y * self.cell_width + PADDING // 2,
